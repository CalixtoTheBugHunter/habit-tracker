export const en = {
  app: {
    title: 'Atomic Habit Tracker',
    loading: 'Loading...',
    error: 'Error: {error}',
    initError: 'Failed to initialize application',
    loadHabitsError: 'Failed to load habits',
    toggleCompletionError: 'Failed to toggle habit completion',
    updateHabitError: 'Failed to update habit',
    deleteHabitError: 'Failed to delete habit',
  },
  habitForm: {
    title: {
      create: 'Create New Habit',
      edit: 'Edit Habit',
    },
    labels: {
      name: 'Name',
      description: 'Description',
      required: '*',
      goalDays: 'Goal days (optional)',
      categories: 'Categories (optional)',
      weekdaySun: 'Sun',
      weekdayMon: 'Mon',
      weekdayTue: 'Tue',
      weekdayWed: 'Wed',
      weekdayThu: 'Thu',
      weekdayFri: 'Fri',
      weekdaySat: 'Sat',
    },
    categories: {
      empty: 'No categories yet. Create some in the Categories view.',
    },
    buttons: {
      create: 'Create Habit',
      update: 'Update Habit',
      cancel: 'Cancel',
      saving: 'Saving...',
    },
    validation: {
      nameRequired: 'Name is required',
    },
    success: {
      created: 'Habit created successfully!',
      updated: 'Habit updated successfully!',
    },
    error: {
      saveFailed: 'Failed to save habit. Please try again.',
      saveFailedGeneric: 'Failed to save habit',
    },
    aria: {
      createForm: 'Create habit form',
      editForm: 'Edit habit form',
    },
    stacking: {
      label: 'Stacked habits',
      placeholder: 'Type to search habits...',
      empty: 'No habits to add.',
      addNewStepHint: 'Press Add or Enter to create as a new step',
      addAria: 'Add habit to stack',
      addButton: 'Add',
      removeAria: 'Remove {name} from stack',
    },
  },
  goalBadge: {
    goalMetAriaLabel: 'Goal met: {completed} of {target} days this week',
  },
  habitList: {
    loading: 'Loading habits...',
    error: 'Error loading habits: {error}',
    empty: 'No habits yet. Start by creating your first habit!',
    noFilterResults: 'No habits match your filters.',
    unnamedHabit: 'Unnamed Habit',
    thisHabit: 'this habit',
    reorderAnnouncement: 'Habit order saved.',
    buttons: {
      markDone: 'Mark as done',
      completed: '✓ Completed',
      updating: 'Updating...',
      edit: 'Edit',
      delete: 'Delete',
      archive: 'Archive',
    },
    aria: {
      markCompleted: 'Mark as completed today',
      markNotCompleted: 'Mark as not completed today',
      editHabit: 'Edit {name}',
      deleteHabit: 'Delete {name}',
      archiveHabit: 'Archive {name}',
      habitNameFallback: 'habit',
      list: 'List of habits',
      reorderHandle: 'Reorder {name}',
    },
    deleteModal: {
      title: 'Delete Habit',
      message: 'Are you sure you want to delete "{name}"? This action cannot be undone.',
      confirm: 'Delete',
      cancel: 'Cancel',
      confirming: 'Deleting...',
    },
    archiveModal: {
      title: 'Archive Habit',
      message: 'Archive "{name}"? You can restore it later from the Archived view.',
      confirm: 'Archive',
      cancel: 'Cancel',
      confirming: 'Archiving...',
    },
    stacking: {
      title: 'Stacked habits',
      expandAria: 'Expand stacked habits for {name}',
      collapseAria: 'Collapse stacked habits for {name}',
      regionAria: 'Stacked habits for {name}',
      unknownHabit: 'Unknown habit',
      checkboxAria: 'Mark {name} as done for today',
      removeModal: {
        title: 'Remove from stack',
        message: 'Remove "{name}" from stacked habits?',
        confirm: 'Remove',
        cancel: 'Cancel',
        confirming: 'Removing...',
      },
      removeButtonAria: 'Remove {name} from stack',
    },
  },
  confirmationModal: {
    defaultConfirm: 'Confirm',
    defaultCancel: 'Cancel',
    defaultProcessing: 'Processing...',
  },
  installPrompt: {
    button: 'Install',
    ariaLabel: 'Install Atomic Habit Tracker app',
    title: 'Install Atomic Habit Tracker app',
  },
  offlineIndicator: {
    badge: 'offline',
    ariaLabel: 'Offline status indicator',
  },
  serviceWorkerUpdate: {
    message: 'A new version is available',
    reloadButton: 'Reload now',
    dismissButton: 'Later',
    ariaLabel: 'Application update available',
    dismissAriaLabel: 'Dismiss update notification',
  },
  streakBadge: {
    ariaLabel: '{streak}-day streak',
  },
  annualCalendar: {
    ariaLabel: 'Annual completion calendar for {name}',
    nameFallback: 'habit',
    today: 'Today, {date}',
    date: '{date}',
    completed: 'Completed, {date}',
    missed: 'Expected but not completed, {date}',
  },
  errorFallback: {
    reloadButton: 'Reload Page',
  },
  errorBoundary: {
    reactRenderError: 'Something went wrong. Please refresh the page.',
  },
  appHeader: {
    menuButtonOpenAria: 'Open menu',
    menuButtonCloseAria: 'Close menu',
  },
  sideMenu: {
    navigationAria: 'Main navigation',
    home: 'Home',
    statistics: 'Statistics',
    archived: 'Archived',
    categories: 'Categories',
    settings: 'Settings',
  },
  categories: {
    title: 'Categories',
    empty: 'No categories yet. Create your first category above!',
    loadError: 'Failed to load categories',
    saveError: 'Failed to save category',
    deleteError: 'Failed to delete category',
    form: {
      titleCreate: 'Create Category',
      titleEdit: 'Edit Category',
      nameLabel: 'Name',
      namePlaceholder: 'e.g. Health',
      required: '*',
      create: 'Create',
      update: 'Update',
      cancel: 'Cancel',
      saving: 'Saving...',
      nameRequired: 'Name is required',
      nameDuplicate: 'A category with this name already exists',
      saveFailed: 'Failed to save category. Please try again.',
      createdSuccess: 'Category created successfully!',
      updatedSuccess: 'Category updated successfully!',
      createAria: 'Create category form',
      editAria: 'Edit category form',
    },
    list: {
      aria: 'List of categories',
      edit: 'Edit',
      delete: 'Delete',
      editAria: 'Edit {name}',
      deleteAria: 'Delete {name}',
    },
    deleteModal: {
      title: 'Delete Category',
      message: 'Delete "{name}"? It will be removed from all habits that use it.',
      confirm: 'Delete',
      cancel: 'Cancel',
      confirming: 'Deleting...',
    },
  },
  categoryBadge: {
    ariaLabel: 'Category: {name}',
  },
  categoryFilter: {
    label: 'Filter by category',
    all: 'All',
    allAria: 'Show all habits',
    chipAria: 'Filter by {name}',
  },
  habitFilter: {
    label: 'Search, filter and sort habits',
    searchLabel: 'Search habits',
    searchPlaceholder: 'Search by name or description...',
    statusLabel: 'Filter by status',
    status: {
      all: 'All',
      completedToday: 'Completed today',
      notCompletedToday: 'Not completed today',
      activeStreak: 'Active streak',
    },
    streakLabel: 'Filter by streak',
    streakChipAria: 'Filter by streak: {label}',
    streak: {
      all: 'All',
      zero: '0',
      oneToSeven: '1–7',
      eightToThirty: '8–30',
      thirtyPlus: '30+',
    },
    sortLabel: 'Sort by',
    sort: {
      manual: 'Manual',
      name: 'Name',
      streak: 'Streak',
      createdDate: 'Creation date',
    },
    clear: 'Clear filters',
    clearAria: 'Clear all search and filter selections',
  },
  archivedHabits: {
    title: 'Archived Habits',
    empty: 'No archived habits.',
    archivedOn: 'Archived on {date}',
    buttons: {
      restore: 'Restore',
      delete: 'Delete',
    },
    aria: {
      list: 'List of archived habits',
      restoreHabit: 'Restore {name}',
      deleteHabit: 'Permanently delete {name}',
      habitNameFallback: 'habit',
    },
    deleteModal: {
      title: 'Permanently Delete Habit',
      message: 'Permanently delete "{name}"? This cannot be undone.',
      confirm: 'Delete',
      cancel: 'Cancel',
      confirming: 'Deleting...',
    },
  },
  statistics: {
    title: 'Statistics',
    empty: 'No habits yet. Create a habit to see statistics!',
    currentStreak: 'Current streak',
    longestStreak: 'Longest streak',
    completionRate: 'Completion rate',
    weeklyRate: 'Last 7 days',
    monthlyRate: 'Last 30 days',
    totalDaysTracked: 'Days tracked',
    daysUnit: 'days',
    goalProgress: 'Goal progress',
    goalProgressValue: '{completed}/{target} this week',
    goalMet: 'Goal met!',
  },
  settings: {
    title: 'Settings',
    close: 'Close settings',
    preferredLanguage: 'Preferred language',
    languageSelectAria: 'Preferred language',
    changelogTitle: 'Changelog',
    changelogBack: 'Back to settings list',
    changelogLoadError: 'Could not load changelog. Check your connection and try again.',
    changelogRetry: 'Try again',
    changelogExternalLinkSuffix: ', opens in new tab',
    theme: {
      title: 'Theme',
      light: 'Light',
      dark: 'Dark',
      system: 'System',
    },
    items: {
      changelog: 'Changelog',
    },
  },
} as const
